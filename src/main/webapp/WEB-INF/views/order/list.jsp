...
<body>
	<main>
		...
		<section>
			<div class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
				...
				<form method="get" action="/Socks/order/list.do">
					...
					<div
						...
						<div
							...
							<select class="form-select w-25" name="year" id="year" required>
								...
								class="form-select w-25" name="month" id="month" required>
					...
					<div
						...
						<div
							...
							<select class="form-select w-50 me-2" name="BrDescription" id="BrDescription">
								<option value="" disabled selected>Brand</option>
								<c:forEach items="${blist}" var="blist">
									<option value="${blist.description}">${blist.description}</option>
								</c:forEach>
							</select> <select class="form-select w-50" name="DuDescription" id="DuDescription">
								<option value="" disabled selected>DutyFree</option>
					...
					<div
						...
						<div
							...
							<!-- 품목명 입력-->
							<select class="form-select w-50 me-2" name="Product_name" id="Product_name">
							...
							<!-- 사이즈 입력 -->
							<select class="form-select w-25 me-2" name="Product_size" id="Product_size">
							...
							<!-- 컬러 입력 -->
							<select class="form-select w-25 me-2" name="Product_color" id="Product_color">
				...
				style="overflow-y: auto; border-collapse: collapse; height: 400px; padding-top: 0px;">
	...
	/* 검색창 고정 */
		$(function() {
			$("#year").val("${param.year}").attr("selected",
					"selected");
		});
		$(function() {
			$("#month").val("${param.month}").attr("selected",
					"selected");
		});
		$(function() {
			$("#BrDescription").val("${param.BrDescription}").attr("selected",
					"selected");
		});
		$(function() {
			$("#DuDescription").val("${param.DuDescription}").attr("selected",
					"selected");
		});
		$(function() {
		...
		$(function() {
			$("#Product_color").val("${param.Product_color}").attr("selectsed",
					"selected");
		});
